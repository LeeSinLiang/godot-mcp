...
func capture_screenshot(params):
    ...
    if debug_mode:
        print("Got viewport: " + str(viewport))
    ...
    if debug_mode:
        print("Got image, size: " + str(img.get_size()))

    # Determine save path
    var save_path = "user://screenshot_temp.png"
    ...
    if not DirAccess.dir_exists_absolute(dir_path):
        ...
        if error != OK:
            printerr("Failed to create directory: " + dir_path + ", error: " + str(error))
            quit(1)

    # Save the screenshot
    var save_error = OK
    if save_path.ends_with(".png"):
        save_error = img.save_png(absolute_path)
    elif save_path.ends_with(".jpg") or save_path.ends_with(".jpeg"):
        save_error = img.save_jpg(absolute_path)
    else:
        # Default to PNG
        save_error = img.save_png(absolute_path)
    ...
    if save_error == OK:
        ...
        if debug_mode:
            print("File exists check: " + str(file_exists))

        if file_exists:
            print("Screenshot saved successfully to: " + absolute_path)
        else:
            printerr("Screenshot reported as saved but file does not exist at: " + absolute_path)
            quit(1)
    else:
